...
module.exports = {
	...
	pdfFilename: 'node',
	symbol_star : config.symbol_star,
	...
	fontStyleBold: 'assets/fonts/GenShinGothic-Bold.ttf',//mingliu.ttf
	lineGap: -5,
	...
	retrievePostData: function(jsonPersonData){
		...
		if(jsonPersonData.g){
			switch(jsonPersonData.g){
				case  "男":
					this.lunarBirth.gender = 1;
				break;
				case  "女":
					this.lunarBirth.gender = 0;
				break;
				default:
					this.lunarBirth.gender = 0;
				break;
		...
		if(jsonPersonData.t){
			switch(jsonPersonData.t){
				case  "天盤":
					this.lunarBirth.type = 0;
				break;
				case  "地盤":
					this.lunarBirth.type = 1;
				break;
				case  "人盤":
					this.lunarBirth.type = 2;
				break;
				default:
					this.lunarBirth.type = 0;
				break;
		...
		switch(this.lunarBirth.type){
			case  0:
				this.lunarBirth.t = "天盤";
			break;
			case  1:
				this.lunarBirth.t = "地盤";
			break;
			case  2:
				this.lunarBirth.t = "人盤";
			break;
			default:
				this.lunarBirth.t = "天盤";
			break;
	...
	createPdf : function(jsonPersonData){
		this.retrievePostData(jsonPersonData);
		var LunarCalendar = require("lunar-calendar");
		this.lunar = LunarCalendar.solarToLunar(this.lunarBirth.AD.y,this.lunarBirth.AD.m,this.lunarBirth.AD.d);
		this.lunarBirth.y.decimal = this.lunar.GanZhiYear[0];
	...
	*
	*/
	setp99_calculate:function(){
		var _this = this;
		for(var n = 0 ; n < 12 ; n++){
			Object.keys(this.pizza[n]).forEach(function(step){
                if(step != "locateTopLeft"){
                	_this.pizza[n][step]['data'].forEach(function(star){
                		if(_this.symbol_star.circle.indexOf(star.substr(2,1)) > -1){
                			_this.pizza[n][step]['symbol'].push("circle");
                		}else if(_this.symbol_star.triangle.indexOf(star) > -1){
                			_this.pizza[n][step]['symbol'].push("triangle");
                		}else{
                			_this.pizza[n][step]['symbol'].push("null");
                		}
                	});
                }
            });
	...
	textVerticalLayout: function(step , alignment){
		var i, r, text, j, elem, next, symbol,
		...
		for( i = 0; i < 12; i++ ){
			...
			for( j = 0; j < elem.data.length; j++){
				...
				text = elem.data[j];
				symbol = elem.symbol[j];
				if( step == "step4"){
					this.doc.font(this.fontStyleBold)
						.fontSize(this.pdfDocument.fontSize).fillColor('purple')
						.text(text,r.x,r.y,{width:this.pdfDocument.fontSize,lineGap:this.lineGap});
				}else{
					this.doc.font(this.fontStyle)
				    	.fontSize(this.pdfDocument.fontSize).fillColor('black')
				    	.text(text,r.x,r.y,{width:this.pdfDocument.fontSize,lineGap:this.lineGap});
				}
				if(!elem.grid[next]){
					//下一個寫入的位置
				...
				if(symbol == "circle"){
					this.circle(r , {x:7,y:41.5});
				}				
				if(symbol == "triangle"){
					//this.circle(r , {x:6.5,y:-6.5});
					this.doc.save().polygon([r.x+0+7, r.y+0-10], [r.x+6.5+7, r.y+6.5*1.732-10], [r.x-6.5+7, r.y+6.5*1.732-10])
				   .lineWidth(1)
				   .fill("red")
				   .stroke("red", "#900");
	...
	*/
	...
	circle: function(r , fix){
		this.doc.circle(r.x + fix.x, r.y + fix.y , 9)
		   .lineWidth(1)
		   .stroke("red", "#900");
	},
    ...
    */
	...
	elements: function(){
		...
		for( i = 0; i < 12; i++ ){
			...
			this.pizza[i].magnitude0.symbol = [];
			...
			this.pizza[i].magnitude1.symbol = [];
			...
			this.pizza[i].magnitude2.symbol = [];
	...
	*/
	profile_calculate: function(){
		...
		if(m == 0){
			...
			switch(this.lunarBirth.gender){
				case 0:
					...
					this.lunarBirth.gender_text = "女";
				break;
				case 1:
					this.profile.data.push( "陽男" );
					this.lunarBirth.clockwise = true;
					this.lunarBirth.gender_text = "男";
				break;
			}
		}else{
			switch(this.lunarBirth.gender){
		...
		var name = this.lunarBirth.name;
		var t = this.lunarBirth.t;
		var g = this.lunarBirth.g;
		var str = sprintf(config.profile_lunar_birth, y , m , d , h );

		this.profile.exportFilename = name+"("+g+")"+"_"+t+"_"+ str;
		this.pdfFilename = this.profile.exportFilename+'.pdf';
